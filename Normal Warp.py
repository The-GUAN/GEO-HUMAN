...
def index_plane(flow, smpl_uv, plane='xy'):

    B, _, H, W = flow.shape
    _, _, N = smpl_uv.shape
    flow = flow.permute(0, 2, 3, 1)
    grid = smpl_uv.permute(0, 2, 1).unsqueeze(2)
...
def shift_4view(f_shift, b_shift, l_shift, r_shift, vs, faces, mask_fb, mask_lr):
    assert vs.dim() == 2 and vs.shape[1] == 3, "vs should be (N, 3)"
    device = vs.device
    B = 1
    f_flow = -f_shift[:, :, :, 2:4] * 2 * warp_scale
    b_flow = -b_shift[:, :, :, 2:4] * 2 * warp_scale
    l_flow = -l_shift[:, :, :, 2:4] * 2 * warp_scale
    r_flow = -r_shift[:, :, :, 2:4] * 2 * warp_scale

    f_flow = f_flow.permute(0, 3, 1, 2).contiguous()
    b_flow = b_flow.permute(0, 3, 1, 2).contiguous()
    l_flow = l_flow.permute(0, 3, 1, 2).contiguous()
    r_flow = r_flow.permute(0, 3, 1, 2).contiguous()

    smpl_vs = vs.clone()
    smpl_vs[:, 1] *= -1
    smpl_vs = smpl_vs[None].permute(0, 2, 1)
    smpl_xy = smpl_vs[:, [0, 1], :]
    flow_f = index_plane(f_flow, smpl_xy, plane='xy')
    flow_b = index_plane(b_flow, smpl_xy, plane='xy')
    flow_f[0, :, mask_fb] = flow_b[0, :, mask_fb]
    smpl_zy = smpl_vs[:, [2, 1], :]
    flow_l = index_plane(l_flow, smpl_zy, plane='zy')
    flow_r = index_plane(r_flow, smpl_zy, plane='zy')
    flow_l[0, :, mask_lr] = flow_r[0, :, mask_lr]

    delta_x = flow_f[0, 0, :]
    delta_y_fb = flow_f[0, 1, :]
    delta_z = flow_l[0, 0, :]
    delta_y_lr = flow_l[0, 1, :]
    delta_y = 0.5 * (delta_y_fb + delta_y_lr)
    ...
    recon_obj = trimesh.Trimesh(new_vs.cpu().numpy(), faces.cpu().numpy(), process=False, maintains_order=True)
    return recon_obj
...
def warp_image(source_image_path, target_image_path):
