...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        gt_image, gt_image_gray,gt_mask = viewpoint_cam.get_image()
        ...
        if gt_image==None:
            cv2.imwrite("...", gt_mask.squeeze().detach().cpu().numpy() * 255)
            threshold = 0.
            gray = rgb_to_grayscale(image)
            soft_binary = torch.sigmoid((gray - 0.5) * 10) 
            cv2.imwrite("....", soft_binary.squeeze().detach().cpu().numpy() * 255)
        ...
        if gt_image!=None:
            ...
            ssim_loss=(1.0 - ssim(soft_binary, gt_mask))
        if gt_image==None:
            
                Ll1 = l1_loss(soft_binary, gt_mask)
        else:
        ...
        # single-view loss
        if iteration > opt.single_view_weight_from_iter and gt_image!=None:
            ...
            depth_normal2 = render_pkg["depth_normal"]
            depth_normal2 = (depth_normal2 + 1) / 2.
            depth_normal_loss = ((1 - depth_mask) * ((depth_normal2 - gt_normal)).abs().sum(0)).mean()
            loss += depth_normal_loss * opt.single_view_weight
        ...
        if iteration > opt.multi_view_weight_from_iter:
            ...
            if nearest_cam is not None:
                ...
                if not opt.wo_use_geo_occ_aware:
                    ...
                    weights[~d_mask] = 0
                if iteration % 200 == 0 and gt_image!=None:
...
def prepare_output_and_logger(args):    
    if not args.model_path:
        ...
        args.model_path = os.path.join("....", unique_str[0:10])
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, app_model):
    ...
    if iteration in testing_iterations:
        ...
        if tb_writer:
            tb_writer.add_histogram("...", scene.gaussians.get_opacity, iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=[10,2_000])
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[10,2_000])
